...
$(document).ready(function() {
	...
	if(divRequest == 'add')  {
		...
		$("#createOrderForm").unbind('submit').bind('submit', function() {
			...
			// form validation 
			if(orderDate == "") {
				$("#orderDate").after('<p class="text-danger"> The วันที่ทำรายการเบิกจ่าย field is required </p>');
			...
			if(clientName == "") {
				$("#clientName").after('<p class="text-danger"> The ชื่อผู้มาเบิก field is required </p>');
		    ...
		    if(productName[x].value == ''){	    		    	
		    	$("#"+productNameId+"").after('<p class="text-danger"> ชื่ออะไหล่ Field is required!! </p>');
	   	...
	   	for (var x = 0; x < quantity.length; x++) {       
		    ...
		    if(quantity[x].value == ''){	    	
		    	$("#"+quantityId+"").after('<p class="text-danger"> ชื่ออะไหล่ Field is required!! </p>');
		...
		// แก้ไขรายการเบิกจ่าย form function
...
function printOrder(orderId = null) {
	...
	if(orderId) {		
        ...
        mywindow.document.write('<html><head><title>รายการการเบิกจ่ายแบบรวมราคา</title>');        
...
function getProductData(row = null) {
	...
	if(row) {
		...
		if(productId == "") {
			...
			// remove check if ชื่ออะไหล่ is selected
			...
			$.ajax({
				...
				success:function(response) {
					...
					// check if ชื่ออะไหล่ is selected
					// var tableProductLength = $("#productTable tbody tr").length;					
					// for(x = 0; x < tableProductLength; x++) {
					// 	var tr = $("#productTable tbody tr")[x];
					// 	var count = $(tr).attr('id');
			...
			success:function(response) {				
				...
				$("#updatePaymentOrderBtn").unbind('click').bind('click', function() {
					...
					if(payAmount && paymentType && paymentStatus) {
			          ...
			          // refresh the จัดการรายการเบิกจ่าย table
								manageOrderTable.ajax.reload(null, false);
